...
export default function Dashboard() {
  ...
  const [pwdShow, setPwdShow] = useState({ cur:false, new:false, conf:false });
  ...
  const savePassword = async () => {
    ...
    try {
      ...
      setPwdShow({ cur:false, new:false, conf:false });
      ...
      <main className="content">
          ...
          <section className="grid cols-2">
            <div className="card profile-form">
              ...
              <div style={{display:'flex', gap:12, marginTop:12}}>
                ...
                <button className="btn ghost" onClick={()=>api.me().then(d=>setProfile({
            ...
            <div className="card profile-form">
              ...
              <label>Current password
                <div style={{display:'flex', gap:8}}>
                  <input type={pwdShow.cur?'text':'password'} value={pwd.current_password}
                         onChange={e=>setPwd({...pwd, current_password:e.target.value})} />
                  <button type="button" className="btn ghost" onClick={()=>setPwdShow({...pwdShow, cur:!pwdShow.cur})}>
                    {pwdShow.cur ? 'Hide' : 'Show'}
                  </button>
                </div>
              </label>
              <label>New password
                <div style={{display:'flex', gap:8}}>
                  <input type={pwdShow.new?'text':'password'} value={pwd.new_password}
                         onChange={e=>setPwd({...pwd, new_password:e.target.value})} />
                  <button type="button" className="btn ghost" onClick={()=>setPwdShow({...pwdShow, new:!pwdShow.new})}>
                    {pwdShow.new ? 'Hide' : 'Show'}
                  </button>
                </div>
              </label>
              <label>Confirm new password
                <div style={{display:'flex', gap:8}}>
                  <input type={pwdShow.conf?'text':'password'} value={pwd.confirm_password}
                         onChange={e=>setPwd({...pwd, confirm_password:e.target.value})} />
                  <button type="button" className="btn ghost" onClick={()=>setPwdShow({...pwdShow, conf:!pwdShow.conf})}>
                    {pwdShow.conf ? 'Hide' : 'Show'}
                  </button>
                </div>
              </label>
              <button className="btn" onClick={savePassword} disabled={pwdSaving}>
                {pwdSaving ? 'Changing…' : 'Change Password'}
              </button>
              <p style={{fontSize:12, color:'#64748b', marginTop:8}}>
                Note: passwords are stored hashed on the server — you can’t view the real password, only change it.
              </p>
            </div>
          </section>
        )}
        ...
        {tab === TABS.ACCOUNT && (
          <section className="card">
            ...
            <p>Change your password, email, and manage sessions here.</p>
